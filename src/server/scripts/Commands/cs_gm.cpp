...
public:
    ...
    static bool HandleGMListIngameCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
        {
            ...
            if ((itr->second->isGameMaster() || (!AccountMgr::IsModeratorAccount(itr_sec) && itr_sec <= AccountTypes(sWorld->getIntConfig(CONFIG_GM_LEVEL_IN_GM_LIST)))) &&
    ...
    static bool HandleGMListFullCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        QueryResult result = LoginDatabase.PQuery("SELECT a.username, aa.gmlevel FROM account a, account_access aa WHERE a.id=aa.id AND aa.gmlevel > 1);
        if (result)
        {
