...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit *unit, const uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit)
    {
        ...
        if (m_caster->IsHostileTo(unit))
        {
            ...
            if ((m_customAttr & SPELL_ATTR0_CU_AURA_CC) && unit->IsControlledByPlayer())if ((m_spellInfo->AttributesCu & SPELL_ATTR0_CU_AURA_CC) && unit->IsControlledByPlayer())
...
void Spell::SearchChainTarget(std::list<Unit*> &TagUnitMap, float max_range, uint32 num, SpellTargets TargetType)
{
    ...
    tempUnitMap.remove(cur.GetTarget());

    while (num)
    {
        TagUnitMap.push_back(cur.GetTarget());
        --num;
        ...
        {
            tempUnitMap.sort(Trinity::ObjectDistanceOrderPred(cur.GetTarget()));
            next = tempUnitMap.begin();
