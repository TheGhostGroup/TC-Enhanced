	...
	if (city->active)
	{
		...
		for (l = city->spawns.begin(); l != city->spawns.end(); ++l)
		{
			...
			if (city->controlled == 1)
			{
				...
				if(CreatureTemplate const *creatureinfo = spawn->creature->GetCreatureInfo())
				{
					const_cast<CreatureTemplate*>(creatureinfo)->faction_A = spawn->faction_A;
					const_cast<CreatureTemplate*>(creatureinfo)->faction_H = spawn->faction_A;
				}
			...
			else if (city->controlled == 0)
			{
				...
				if(CreatureTemplate const *creatureinfo = spawn->creature->GetCreatureInfo())
				{
					const_cast<CreatureTemplate*>(creatureinfo)->faction_A = spawn->faction_H;
					const_cast<CreatureTemplate*>(creatureinfo)->faction_H = spawn->faction_H;
				}
